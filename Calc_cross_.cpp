...
struct constant_group {
    ...
    int random_seed_antibiotics_evo = 0;
    int random_seed_antibiotics_test = 0;
    double alpha = 0.0;
    int CF_index = 0;
    int Import_Max_Evolution_Time = 0;
...
void Make_Antibiotics(constant_group& cg)
{
    mt19937_64 mt(cg.random_seed_antibiotics_test);
...
int main(int argc, char* argv[])
{
    ...
    cg.random_seed_antibiotics_evo = atoi(argv[4]);
    cg.random_seed_antibiotics_test = atoi(argv[5]);
    ...
    ofstream outputfile_fitness("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/Cross_fitness_" + to_string(cg.random_seed_antibiotics_evo) + "_" + to_string(cg.random_seed_antibiotics_test) + "_.csv");
    ...
    for (int i = 0; i < cg.N_selection; ++i) {
        if (import_Genome(Genome_selection[i], "./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/Anti_Genome_" + to_string(cg.random_seed_antibiotics_evo) + "_" + to_string(i) + "_.csv") == 0) {
            return 0;
        }
    }
    for (int i = 0; i < cg.N_selection; ++i) {
        outputfile_fitness << Calc_fitness(cg, Genome_selection[i], x[i]) << endl;
    }
    return 0;
}