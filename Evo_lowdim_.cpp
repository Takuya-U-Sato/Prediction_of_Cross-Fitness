...
void Mutation(constant_group& cg, vector<vector<int>>& Genome_cell, int evolution_time, int cell_index)
{
    mt19937_64 mt(100000000 + cg.random_seed_genotype * cg.Max_Evolution_Time * cg.N_cell + evolution_time * cg.N_cell + cell_index);
    uniform_int_distribution<> rnd_int(0, 1);
    uniform_real_distribution<> rnd_01(0.0, 1.0);
    for (int i = 0; i < cg.N_gene + cg.N_target; ++i) {
        for (int j = 0; j < cg.N_gene + cg.N_input; ++j) {
...
void Make_IO(constant_group& cg)
{
    ...
    ofstream outputfile_input("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/Input_.csv");
    ofstream outputfile_target("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/Target_.csv");
    ...
    outputfile_input << 0;
    for (int i = 0; i < cg.N_input; ++i) {
        outputfile_input << "," << 0.0;
    }
    outputfile_input << endl;
    outputfile_target << 0;
    for (int i = 0; i < cg.N_target; ++i) {
        outputfile_target << "," << 0.5;
    }
    outputfile_target << endl;
    for (int i = 0; i < cg.Rank_lowdim; ++i) {
        for (int k = 1; k <= 2; ++k) {
            outputfile_input << 2 * i + k;
            for (int j = 0; j < cg.N_input; ++j) {
                tmp_input[j] = (2 * (k - 1) - 1) * save_input[i][j];
                outputfile_input << "," << tmp_input[j];
            }
            outputfile_input << endl;
            cg.Input.emplace_back(tmp_input);
            outputfile_target << 2 * i + k;
            for (int j = 0; j < cg.N_target; ++j) {
                tmp_target[j] = 0.5 + cg.alpha * (2 * (k - 1) - 1) * save_target[i][j];
                outputfile_target << "," << tmp_target[j];
            }
            outputfile_target << endl;
            cg.Target.emplace_back(tmp_target);
        }
...
int main(int argc, char* argv[])
{
    ...
    CheckTheFolder::checkExistenceOfFolder("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype));
    ...
    for (int i = 0; i < cg.N_selection; ++i) {
        Make_genome(cg, Genome_selection[i], i + cg.N_selection * cg.random_seed_genotype);
    }
    ...
    ofstream outputfile_fitness("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/fitness_.csv");
    ...
    for (int evolution_time = 1; evolution_time <= cg.Max_Evolution_Time; ++evolution_time) {
        ...
        for (cg.CF_index = 0; cg.CF_index < 2 * cg.Rank_lowdim + 1; cg.CF_index += 1) {
#pragma omp parallel for
        ...
        if (evolution_time % cg.Max_Evolution_Time == 0) {
            for (int i = 0; i < cg.N_selection; ++i) {
                ofstream outputfile_Genome(
                    "./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/Genome_" + to_string(i) + "_.csv");
