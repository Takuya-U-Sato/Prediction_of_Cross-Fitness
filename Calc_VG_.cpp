...
void Mutation(constant_group& cg, vector<vector<int>>& Genome_cell);
...
void Mutation(constant_group& cg, vector<vector<int>>& Genome_cell)
{
    mt19937_64 mt(cg.random_seed_mutation);
...
int main(int argc, char* argv[])
{
    ...
    ofstream outputfile_fitness("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/VG_fitness_" + to_string(cg.index_cell) + "_.csv");
    ofstream outputfile_x("./Np_" + to_string(cg.Rank_lowdim) + "_" + to_string(cg.alpha) + "_" + to_string(cg.random_seed_genotype) + "/VG_x_" + to_string(cg.index_cell) + "_.csv");

    cg.Input.emplace_back(vector<double>(cg.N_input, 0.0));
    cg.Target.emplace_back(vector<double>(cg.N_target, 0.5));
    for (cg.random_seed_mutation = 0; cg.random_seed_mutation < 10000; cg.random_seed_mutation += 1) {
        Genome[cg.index_cell] = Genome_selection[cg.index_cell];
        Mutation(cg, Genome[cg.index_cell]);
        outputfile_fitness << Calc_fitness(cg, Genome[cg.index_cell], x[cg.index_cell]) << endl;
